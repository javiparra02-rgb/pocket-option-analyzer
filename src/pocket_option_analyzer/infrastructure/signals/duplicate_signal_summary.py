...
class DuplicateSignalSummary:
    ...
    def __post_init__(
        ...
        if (
            ...
            is SignalDirection.NONE
        ):
            raise ValueError(
                "accepted_direction no puede ser NONE."
            )
        ...
        if self.duplicate_suppressed_count < 1:
            raise ValueError(
                "duplicate_suppressed_count debe ser "
                "mayor o igual a 1."
            )

        if self.call_duplicate_count < 0:
            raise ValueError(
                "call_duplicate_count no puede ser negativo."
            )

        if self.put_duplicate_count < 0:
            raise ValueError(
                "put_duplicate_count no puede ser negativo."
            )
        ...
        if (
            ...
            raise ValueError(
                "Los conteos CALL y PUT deben coincidir "
                "con duplicate_suppressed_count."
            )
        ...
        if (
            ...
            raise ValueError(
                "last_duplicate_at no puede ser anterior "
                "a first_duplicate_at."
            )
    ...
    def add(
        ...
        if (
            record.candle_interval_started_at
            != self.candle_interval_started_at
        ):
            raise ValueError(
                "La señal duplicada pertenece a otro intervalo."
            )
    ...
    def _validate_duplicate_record(
        ...
        if (
            record.disposition
            is not SignalRecordDisposition.DUPLICATE_SUPPRESSED
        ):
            raise ValueError(
                "El registro debe ser una señal "
                "duplicada suprimida."
            )
        ...
        if (
            ...
            raise ValueError(
                "Una señal duplicada no puede tener "
                "dirección NONE."
            )